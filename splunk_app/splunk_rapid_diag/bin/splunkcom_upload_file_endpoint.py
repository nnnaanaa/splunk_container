...
from rapid_diag.util import get_splunkhome_path, censor_http_log
...
def handle_kverror(e: Union[KeyError, ValueError] , sn_upload: SimpleNamespace) -> None:
    """ Key/Value error handler """
    if sn_upload and sn_upload.upload_user and sn_upload.upload_password:
        _LOGGER.error("Value error check logs for more info %s",
        censor_http_log(str(e), sn_upload.upload_user, sn_upload.upload_password))
    else:
        _LOGGER.error("Value error while parsing payload")
...
class SplunkcomUploadFileEndpoint(PersistentServerConnectionApplication):
    ...
    def _handle(self, args : JsonObject) -> JsonObject:
        ...
        try:
            ...
            real_path_file = os.path.realpath(upload_file)
            ...
            if not str(real_path_file).startswith(DEFAULT_OUTPUT_ROOT) or not regexp.search(os.path.basename(real_path_file)):
            ...
            check_permission = info_gather.splunkcom_upload_permitted(real_path_file, sn_upload)
            ...
            SPLUNK_BIN_PATH, [SPLUNK_BIN_PATH, 'cmd', RAPIDDIAG_CLI_PATH, '--token_auth', 'upload', real_path_file,
