...
def censor_plain_and_base64(needle: str, haystack: str) -> str:
    newval = len(needle) * '*'
    haystack = haystack.replace(needle, newval)
    haystack = haystack.replace(base64.b64encode(needle.encode()).decode(), newval)
    return str(haystack)

def censor_http_log(msg: str, user: str, password: str) -> str:
    return str(censor_plain_and_base64(user + ':' + password,
        censor_plain_and_base64(user, censor_plain_and_base64(password,msg))))
