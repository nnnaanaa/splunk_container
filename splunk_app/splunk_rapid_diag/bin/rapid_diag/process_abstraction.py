...
class Process(ProcessMatch, Serializable):
    """ Class representing a process (and it's properties) in the OS we want to work with
        in collectors.
    """
    def __init__(self, name : str, pid : int, ppid : int, args : str, process_type : str,
                 show_in_simple_listing : bool) -> None:
        ...
        self.show_in_simple_listing : bool = show_in_simple_listing
    ...
    def to_json_obj(self) -> JsonObject:
        return {
            ...
            'show_in_simple_listing': self.show_in_simple_listing
        }
    ...
    def from_json_obj(obj : JsonObject) -> Serializable:
        return Process(str(obj['name']), int(obj['pid']), int(obj['ppid']), str(obj['args']), str(obj['process_type']), False)
...
class ProcessLister:
    ...
    def build_process_from_args(args : argparse.Namespace) -> Process:
        ...
        return ProcessLister.build_process(args.name, args.pid, args.ppid, False, [args.args])

    @staticmethod
    def build_process(name : str, pid : int, ppid : int, show_in_simple_listing: bool, args_l : List[str]) -> Process: # pylint: disable=too-many-branches
        ...
        if name and args:
            if name == 'splunkd':
                if 'search' in args:
                    ...
                    if match:
                        return SearchProcess.load_from_disk(Process(name, pid, ppid, args,
                                                            'splunk search', show_in_simple_listing), match.group(1))
                    process_type = 'splunk search runner'
                elif 'process-runner' in args:
                    process_type = 'splunk process-runner'
                elif (' -p ' in args and ('start' in args
                    or '_internal_launch_under_systemd' in args)) or args.endswith(' service'):
        ...
        return Process(name, pid, ppid, args, process_type, show_in_simple_listing)
    ...
    def _build_process_windows(pid : int) -> Process:
        ...
        wmi_obj = wmi.WMI()
        show_in_simple_listing = False

        process = wmi_obj.Win32_Process(processid=pid)[0]
        if str(process.ExecutablePath).startswith(get_splunkhome_path(['bin'])):
            show_in_simple_listing = True
        ...
        return ProcessLister.build_process(name, int(pid), int(ppid), show_in_simple_listing, args)
    ...
    def _build_process_linux(pid : int) -> Process:
        show_in_simple_listing = False
        exe_path = os.path.normpath(os.readlink(os.path.join('/proc', str(pid), 'exe')))
        if exe_path.startswith(get_splunkhome_path(['bin'])):
            show_in_simple_listing = True
