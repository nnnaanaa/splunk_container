...
class FileWrappingProgressRD():
    ...
    def __init__(self, readable_obj: IO, read_max: int = None, options: SimpleNamespace = None) -> None: # pylint: disable=unused-argument
        ...
        self.bytes_read = 0
        if options:
            self.case_number = options.case_number
    ...
    def _update(self) -> None:
        ...
        if not os.path.exists(self.name):
            with open(self.name, "w") as f:
                data = {'case_number': self.case_number, 'percent':cur_percent,
                    'start_time':str_now, 'updated': str_now }
                json.dump( data, f, indent=4 )
        else:
            with open(self.name, "r+") as f:
                data = json.load(f)
                # if data['percent'] == 100:
                #     data['start_time'] = str_now
                data['case_number'] = self.case_number
                data['percent'] = cur_percent
                data['updated'] = str_now
                if cur_percent == 0:
                    data['start_time'] = str_now
                    data['pid'] = os.getpid()
                if cur_percent > 0:
                    remaining = 100 - cur_percent
                    timetaken = datetime.now() - datetime.strptime(data['start_time'], '%m/%d/%Y, %H:%M:%S')
                    data['eta'] = int((timetaken.seconds / cur_percent) * remaining)
                if cur_percent == 100:
                    data.pop('pid', None)
                f.seek(0)
                json.dump( data, f, indent=4 )
