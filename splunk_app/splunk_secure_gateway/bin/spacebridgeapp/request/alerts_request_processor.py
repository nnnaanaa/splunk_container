...
async def populate_alerts_list_response(log: logging.Logger,
    ...
    alert_ids = await fetch_alert_ids(log,
    ...
    log.debug("Fetched alert_ids, alert_ids=%s", alert_ids)
...
async def fetch_alert_ids(log: logging.Logger,
                          request_context,
                          order=None,
    ...
    alert_ids = [alert["alert_id"] for alert in response_json]

    log.debug("fetch_alert_ids result, alert_ids=%s", alert_ids)
...
async def process_alerts_clear_request(log: logging.Logger,
                                       request_context, _client_single_request, server_single_response, async_kvstore_client):
    """
