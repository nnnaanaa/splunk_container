...
async def process_subscribe_request(log: logging.Logger,
    ...
    if client_subscription_message.clientSubscribeRequest.HasField(CLIENT_SUBSCRIBE_DASHBOARD_VISUALIZATION_REQUEST):
        log.info("type=CLIENT_SUBSCRIBE_DASHBOARD_VISUALIZATION_REQUEST")
        ...
        log.info("type=CLIENT_SUBSCRIBE_SAVED_SEARCH_REQUEST")
        ...
        log.info("type=CLIENT_SUBSCRIBE_DASHBOARD_INPUT_SEARCH_REQUEST")
        ...
        log.info("type=CLIENT_SUBSCRIBE_UDF_DATASOURCE")
        ...
        log.info("type=CLIENT_SUBSCRIBE_GENERIC_MESSAGE")
...
async def process_subscribe_update_request(log: logging.Logger,
    ...
    if client_subscription_update.HasField(CLIENT_SUBSCRIBE_DASHBOARD_VISUALIZATION_CLUSTER_MAP):
        log.info("type=CLIENT_SUBSCRIBE_DASHBOARD_VISUALIZATION_CLUSTER_MAP")
        ...
        log.info("type=CLIENT_SUBSCRIBE_DASHBOARD_VISUALIZATION_CHOROPLETH_MAP")
        await process_dashboard_visualization_map_update(log,
                                                         request_context=request_context,
        ...
        log.info("type=CLIENT_SUBSCRIBE_UPDATE_GENERIC_MESSAGE")
...
async def process_dashboard_visualization_map_update(log: logging.Logger,
    ...
    log.debug('Start process_subscription_update subscription_id=%s', subscription_id)
...
async def lazy_load_subscription_search(log: logging.Logger,
                                        request_context, input_tokens, search_key,
    ...
    if search_defn.base:
        ...
        log.debug("Base search exists, base=%s", parent_search_defn)
    ...
    print('lewl 3', async_splunk_client)
    job_status = await get_search_job_content(log,
                                              ...
                                              params=params)
    if not job_status:
        log.debug("Start search job, sid=%s", sid)
...
async def create_subscription(log: logging.Logger,
    ...
    log.info("Subscription created. subscription_id=%s, search_key=%s", subscription_id, search_key)
...
async def process_unsubscribe_request(log: logging.Logger,
    ...
    server_subscription_response.subscriptionId = subscription_id
    log.debug('Start process_unsubscribe_request subscription_id=%s', subscription_id)
    ...
    if response.code == HTTPStatus.NOT_FOUND:
        ...
        error = await response.text()
        log.debug("Subscription Id not found. status_code=%s, error=%s, subscription_id=%s",
                     response.code, error, subscription_id)
    elif response.code != HTTPStatus.OK:
        error = await response.text()
        log.error("Failed to unsubscribe. status_code=%s, error=%s, subscription_id=%s",
                     response.code, error, subscription_id)
        raise SpacebridgeApiRequestError(error, status_code=response.code)

    log.info("Finished process_unsubscribe_request subscription_id=%s", subscription_id)
...
async def process_subscription_update(log: logging.Logger,
    ...
    log.debug('Start process_subscription_update subscription_key={}'.format(subscription.key()))
    ...
    log.debug("Subscription Post Search Updated subscription_key={}, post_search={}"
