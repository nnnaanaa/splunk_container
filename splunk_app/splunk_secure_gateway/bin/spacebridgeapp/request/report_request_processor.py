...
async def process_report_get_request(log: logging.Logger,
    ...
    report_detail = _parse_single_report_entry(report_json, next_scheduled_time_parser)
...
async def process_reports_list_request(log: logging.Logger,
    ...
    next_scheduled_time_parser = await _async_get_next_scheduled_time_parser(request_context, async_splunk_client)
    report_details = _parse_reports_response(jsn, next_scheduled_time_parser)
...
def _parse_reports_response(jsn, next_scheduled_time_parser) -> [ReportDetail]:
    """
    ...
    for entry in jsn["entry"]:
        report = _parse_single_report_entry(entry, next_scheduled_time_parser)
        response.append(report)

    return response

def _parse_single_report_entry(jsn, next_scheduled_time_parser) -> ReportDetail:
    """
    Parse json object returned by Splunk and returns a ReportDetail object
    """
