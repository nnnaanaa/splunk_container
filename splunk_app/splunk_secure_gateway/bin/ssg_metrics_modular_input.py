...
class MetricsModularInput(BaseModularInput):
    ...
    def do_run(self, input_config):
        ...
        logger = self.setup_logging('secure_gateway_metrics.app', filename=f"{constants.SPACEBRIDGE_APP_NAME}_metrics.log")
        ...
        logger.info("Running secure gateway metrics modular input")
        if not modular_input_should_run(self.session_key, logger=logger):
            logger.debug("Modular input will not run on this node.")
            return
