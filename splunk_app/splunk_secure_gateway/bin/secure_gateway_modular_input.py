...
async def _periodic_spacebridge_server_update(websocket,
    ...
    while not websocket.closed:
        ...
        current_timestamp = config.get_last_updated()
        config = load_config(auth_header.session_token, force_refresh=True)
        refreshed_timestamp = config.get_last_updated()

        # Restart if timestamp is found for the first time (first update scenario)
        # or if refreshed timestamp > old timestamp
        if (current_timestamp is None and refreshed_timestamp is not None) or \
            (current_timestamp is not None and refreshed_timestamp is not None and refreshed_timestamp > current_timestamp):
            try:
                logger.info("Detected config change, restarting modular inputs.")
                ...
                logger.error(f"Failed to restart modular input after detecting config change, error:{e}")
...
class SpacebridgeModularInput(BaseModularInput):
    ...
    def do_run(self, input_config):
        ...
        logger = self.setup_logging('secure_gateway_modular_input.app')
...
class WebsocketContext(AbstractWebsocketContext):
    ...
    async def on_open(self, protocol):
        ...
        asyncio.create_task(_periodic_spacebridge_server_update(protocol, self.auth_header, self.logger, self.config))
