...
from spacebridgeapp.rest.devices.alert_helper import preprocess_payload, build_alert, persist_alert, \
    persist_recipient_devices, send_push_notification
...
async def do_trigger(config: SecureGatewayConfig, log: logging.Logger, alert_payload):
    ...
    if response and "_key" in response:
        ...
        log.info("persist_recipient_devices started alert_id=%s, devices=%s", alert_id, len(registered_devices))
        await persist_recipient_devices(log,
                                        request_context, alert_id, registered_devices, alert_payload,
                                        async_kvstore_client)
        log.info("persist_recipient_devices ok alert_id=%s, devices=%s", alert_id, len(registered_devices))
        log.info("send_push_notifications starting registered_devices=%s", len(registered_devices))
